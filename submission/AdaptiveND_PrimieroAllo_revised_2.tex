...
%opening
\title{{\scriptsize Forthcoming in Can Ba\c{s}kent \& Thomas M. Ferguson (eds.)\\
\textit{Graham Priest on Dialetheism and Paraconsistency},\\
Outstanding Contributions in Logic, Springer.\\}
Annotated Natural Deduction for Adaptive Reasoning}
\author{Patrick Allo\thanks{Centre for Logic and Philosophy of Science, Vrije Universiteit Brussel \&
Oxford Internet Institute, University of Oxford.
Supported by the European Union’s Horizon 2020 research and innovation programme under the Marie Sk\l{}odowska-Curie Grant Agreement No. 657017.}
\and Giuseppe Primiero\thanks{ Department of Computer Science, Middlesex University London.}}
\date{}
...
The derivability sign is enhanced with a signature {\sf s} that corresponds to a counter of the ordered derivation steps executed to obtain the corresponding ND-formula in a tree. This annotation plays a role in the extension of the calculus developed in Section \ref{sec:adaptive}.
...
\begin{proposition}\label{prop:choice}
    If $\Turn{\Gamma;\cdot}{\Phi(\Gamma)}$ occurs in a proof and $\Lambda$ is the set of all $\Delta^{min}$ derived up to stage $s$ that are minimal at $s$, then each choice-set in $\Phi(\Gamma)$ is a subset of a choice-set from $\Lambda$.
\end{proposition}
\noindent\textsl{Proof.}~If $\Gamma; \cdot \vdash_{s - n  + (i-1)} \Delta_i^{min}$ is a premise used to derive $\Turn{\Gamma;\cdot}{\Phi(\Gamma)}$, adding a premise $\Gamma; \cdot \vdash_{s - n  + (j-1)} \Delta_j^{min}$ for some $\Delta_j$ that was already derived at some stage $s - (n + m)$ would not lead to the violation of the side-condition for the application of \textsc{Choice}. Let $\Turn{\Gamma;\cdot}{\Phi'(\Gamma)}$ be the conclusion obtained by adding this premise, and note that each choice-set in $\Phi'(\Gamma)$ can be obtained by extending a choice-set from $\Phi(\Gamma)$ with some member of $\Delta_j$.\qed
...
such that for each ${\sf choice_{i}}(\{\Delta_{1},\ldots, \Delta_{n}\})^{min} \in \Phi(\Gamma)$:
...
such that for each $\Theta_i \in \langle\Theta_1, \ldots, \Theta_n\rangle$ we have:
\[
    \Theta_i \cap \mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\})^{min} \ne \emptyset
    \tag{\dag}
\]

and there is no $\Theta' \not\in \{\Theta_1, \ldots, \Theta_n\}$ for which
\[
    \Gamma; \Theta' \vdash_{\mathsf{t < s}} \psi. \tag{\ddag}
\]
...
Let us now extend the example from Section \ref{sec:example} with a new branch to illustrate the derivation step obtained by applying the Marking Rule $\XBox$R. Let $\mathbb{D}$ be the derivation from our initial example that ended with the derivation at stage {\sf 5} of $q$ in context $\Gamma$ and with $(p \wedge \neg p)^-$ as a condition. We extend it now as follows:
...
At stage {\sf 13} the formula $r$ is no longer derived, because its adaptive condition is in the set of unreliable formulas derived at stage \textsf{12}. Now we can provide a further extension of this derivation dubbed $\mathbb{D^{\prime\prime}}$:
...
We illustrate the procedure by reconstructing the linear proof that corresponds to the example from Sections \ref{sec:example} and \ref{sec:example2}:
\begin{center}
    \begin{tabular}{cllcl}
...
\begin{enumerate*}
    ...
    \item If $\phi \wedge \neg \phi$ is the result of ($\wedge$I), each of its conjuncts should be derivable. We focus on the proof-paths to formulae of the form $\neg \phi$, of which there are four:
        ...
        \end{description}
\end{enumerate*}
Consequently, for every abnormality $\phi \wedge \neg \phi$ that occurs in a minimal disjunction of abnormalities, we can show that $\phi$ must occur as the negative part of some premise. A fortiori, this means that only abnormalities that can be formed from a member of $\Omega(\Gamma)$ can occur in a minimal disjunction of abnormalities.\qed
