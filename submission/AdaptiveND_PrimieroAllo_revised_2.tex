...
\begin{proposition}\label{prop:unrel}
    If $\Turn{\Gamma;\cdot}{\Unrel(\Gamma)}$ occurs in a proof and $\Lambda$ is the set of all $\Delta^{min}$ derived up to stage $s$ that are still minimal at $s$, then $\Unrel(\Gamma) \subseteq \bigcup(\Lambda)$.
\end{proposition}
