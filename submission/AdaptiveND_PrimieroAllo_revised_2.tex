...
%\footnote{As mentioned above, the current setting of {\sf AdaptiveND} is specified for an inconsistency-adaptive logic.}
...
{\color{red}An {\sf AdaptiveND} proof is a sequence of {\sf AdaptiveND} trees $T_1, \ldots, T_{i}, \ldots T_n$ with $i$ the highest numbered judgement of tree $T_i$.}
\end{definition}
...
\begin{proposition}\label{prop:minchoice}
    {\color{red}If $\Turn{\Gamma;\cdot}{{\sf choice_{i}}(\{\Delta_{1},\ldots, \Delta_{n}\})^{min}}$ occurs in a proof and $\Lambda$ is the set of all $\Delta^{min}$ derived up to stage $s$ that are still minimal at $s$, then $\sf{choice_{i}}(\{\Delta_{1},\ldots, \Delta_{n}\})^{min}$ is a subset of some minimal choice-set from $\Lambda$, and for each minimal choice-set from $\Lambda$ there is a ${\sf choice_{j}}(\{\Delta_{1},\ldots, \Delta_{n}\})^{min}$ it is a superset of.}
\end{proposition}
...
{\TurnMarkedMAflex{max(s, s')+1}{\Gamma; \Theta^{-}}{\psi}}
\end{mathpar}

with
\[
    \Theta \cap \mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\})^{min} \ne \emptyset, \tag{\dag}
\]

and
{\color{red}
\[
    {\Gamma; \Theta' \vdash_{\mathsf{t < s'}} \psi \Longrightarrow \Theta' \cap \mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\})^{min} \ne \emptyset \tag{\ddag}}
\]}
...
\begin{mathpar}
\infer*[right=$\XBox$M2]{\Turn{\Gamma; \Theta^-}{\psi}
...
\Theta \cap \mathsf{choice_i} \ne \emptyset\\ {\color{red}\ddag}} 
...
{\color{red} replace daggers with asterisks}
...
\end{mathpar}
Next, let $\mathbb{B}$ be the following derivation, where $\dag$ refers to the application of $\wedge$ I followed by the application of \textsc{MinDab}, and $\ddag$ refers to the application of \textsc{Choice} followed by the application of \textsc{MinChoice} (boldface is used to indicate which member of a minimal disjunction of abnormalities is chosen):
...
\begin{mathpar}
    \infer*[right=$\ddag$]
        ...
        {\TurnTwelve{\Gamma; \cdot}{\mathsf{choice_1}(\{\{\mathbf{p \wedge \neg p}, q \wedge \neg q\}\})^{min}}}
\end{mathpar}
...
\end{mathpar}
At this point, we can see that neither of the following attempts to mark conditional deductions of $r$ using the minimal abnormality marking-rule can succeed:
\begin{mathpar}
\infer*[right=\XBox M2]
    {
        \infer*[right=RC]
        ...
        \infer*[right=MinChoice]
        {\mathbb{B'}}
        {\TurnPrime{\Gamma; \cdot}{\mathsf{choice_1}(\{\{\mathbf{p \wedge \neg p}, q \wedge \neg q\}\})^{min}}}
    }
    {?}
\end{mathpar}
\begin{mathpar}
\infer*[right=\XBox M2]
    ...
    {?}
\end{mathpar}
{\color{red} double read this}
for in the former case judgement 18 depends on a condition that does not depend on the normal behaviour of $p \wedge \neg p$, whereas in the latter case judgement $\mathsf{s}$ (a copy of judgement 5) does not depend on the normal behaviour of $q \wedge \neg q$. As a consequence, neither judgement 18 or judgement $\mathsf{s}$ can be marked at a stage later than 18.

Finally, do remark that the failure to apply \XBox M2 after the derivation of judgement 18 does not depend on the presence of the judgement $\mathsf{s}$ where $\mathsf{s} > 13$. The side-condition for the application of this rule is already violated by the presence of judgement 5 higher-up in the proof; even though the latter was marked at stage 13.
...
\end{definition}
%\marginpar{$\{\psi \mid \psi\subset\phi\}$\\This doesn't work: formulas are not sets.}
...
\begin{theorem}\label{thm:subform}
    ...
    % \marginpar{If $\Gamma;\cdot \vdash\bigvee(\Delta)$, then $\Delta \subseteq \Omega(\Gamma)$\\ We need it to be minimal, but our notation $\bigvee(\Delta)^{min}$ only applies to stages, so we no object-level language to say this.}
\end{theorem}
