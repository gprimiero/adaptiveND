...
$\Phi(\Gamma)$ derived at stage {\sf s} from $\Gamma$ denotes the set of choice sets of $\{\Delta_{1}, \dots, \Delta_{n}\}$ derived from $\Gamma$ up to stage {\sf s-1} according to {\sf MinDab}. We denote each such choice set with ${\sf choice_{i}}(\{\Delta_{1}, \dots, \Delta_{n}\})$.
\end{definition}
...
\end{itemize}
%
%In the first case, one considers all possible abnormal formulas to be invalid; in the second case, one tries to minimize the number of such unavoidable contradictions. 
In this section, we extend {\sf AdaptiveND} with rules corresponding to both strategies by providing a proof-theoretic equivalent of the standard marking conditions. The conclusion of a Marking Rule expresses a dead-end for the tree, and as such its content should not be used as premise for any other step. Nonetheless, we wish to preserve all the information obtained in the tree, including retracted formulas, also in order to extract a linear proof from it: therefore, we allow marked judgements to be unused premises for any rule.
...
}{\TurnMarkedThirteenREL{\Gamma; \cdot}{r}}
\end{mathpar}
\bigskip
...
In the following example, we do not derive set of unreliable formulas when there is a single minimal disjunction of abnormalities. Let $\Gamma = \{p \vee q, \neg q,\\
(q \wedge \neg q) \vee (r \wedge \neg r), (q \wedge \neg q) \to (r \wedge \neg r),\\
(q \wedge \neg q) \vee (s \wedge \neg s), (q \wedge \neg q) \to (s \wedge \neg s)\}$\\
