...
{\TurnMaxTwoPlusOne {\Gamma, \phi_{1}; \phi^{-}} {\phi_{2}}}
\and
...
\begin{enumerate}
\item  minimal disjunctions of formulas of type $\Omega$, denoted by $\bigvee(\Delta)^{min}$;
\item the union of all $\bigvee(\Delta)^{min}$ derived up to a certain stage from $\Gamma$, denoted by $\bigcup\Delta(\Gamma)$;
\item the set of minimal choice sets of $\bigvee(\Delta)^{min}$ derived up to a certain stage from $\Gamma$, denoted by $\Phi(\Gamma)$.
\end{enumerate}
...
$\bigcup\Delta(\Gamma)$ derived at stage {\sf s} from $\Gamma$ denotes the set of all $\{\Delta_{1}, \dots, \Delta_{n}\}$ derived from $\Gamma$ up to stage {\sf s-1} according to {\sf MinDab}.
\end{definition}
...
$\Phi(\Gamma)$ derived at stage {\sf s} from $\Gamma$ denotes the set of minimal choice sets of $\{\Delta_{1}, \dots, \Delta_{n}\}$ derived from $\Gamma$ up tp stage {\sf s-1} according to {\sf MinDab}. We denote each such choice set with $\{\Delta^{min}_{i\dots n}\}$.
\end{definition}
...
\begin{figure}[t]
\begin{mathpar}
\infer*[right=$\XBox$R]{\Turn{\Gamma;\phi^{-}}{\psi}\\ \TurnPrime{\Gamma; \cdot}{\bigcup\Delta(\Gamma)} \\ {\phi \in \bigcup\Delta(\Gamma)}}
{\TurnMaxPlusOneREL{\Gamma;\phi^{-}}{\psi}}
\end{mathpar}
...
The first marking rule $\XBox$M reflects the following condition: if a formula $\psi$ is derived at stage $s$ under an adaptive condition $\phi$ which is part of all $\Delta_{i}^{min} \in \Phi(\Gamma)$ at stage $s'$, then at the next stage the formula $\psi$ can be retracted.

The second marking rule $\XBox$M2 reflects the following condition: if a formula $\psi$ is derived always under an adaptive condition which is part of the same $\{\Delta_{i\dots n}^{min}\} \in \Phi(\Gamma)$, then at the next stage the formula $\psi$ can be retracted at all stages where it was previously derived. Here the adaptive condition of the conclusion should be intended as saying that the retraction applies to all stages where the derivation of $\psi$ was obtained under one of the listed conditions.
...
\begin{mathpar}
\infer*[right=$\XBox$M2]{\TurnMinusn{\Gamma;\phi^{-}}{\psi}\\
\ldots\\
\TurnMinusOne{\Gamma; \xi^{-}}{\psi}\\
\Turn{\Gamma; \cdot}{\Phi(\Gamma)}\\
{\exists \{\Delta_{i\dots n}^{min}\}\in \Phi(\Gamma), \phi^{-}, \dots, \xi^{-}\in \{\Delta_{i\dots n}^{min}\}}}
{\TurnPlusOneMarkedMA{\Gamma; \phi^{-}, \dots, \xi^{-}}{\psi}}
\end{mathpar}
