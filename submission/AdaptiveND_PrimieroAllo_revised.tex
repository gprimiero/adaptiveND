...
\infer*[right=\textsc{MinChoice}]{\infer*[right=\textsc{Choice}]{\Pi}{\Turn{\Gamma, \cdot}{\Phi(\Gamma)}}}
{\Turn{\Gamma; \cdot}{\mathsf{choice_1}(\{\Delta_1, \ldots, \Delta_n\})}}\\ \ldots\\ \infer*{\Pi}{\vdots}\\\Theta \cap \mathsf{choice_i} \ne \emptyset}
{\TurnMarkedMA{\Gamma;\Theta^-}{\psi}}
\end{mathpar}
...
\begin{description}
    ...
    \item[MinAb2] for every derivable $\Gamma; \Theta^{-}\vdash_{{\sf s}} \phi, \dots,\Gamma; \Theta'^{-}\vdash_{{\sf s}} \phi, $, if any $\mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\})$ derivable at that stage intersects with all $\Theta, \dots, \Theta'$, then there is a tree $T_{j>i}\in P$ such that ends with $\Gamma; \Theta^{-}\vdash_{{\sf t\XBox}} \phi$.
