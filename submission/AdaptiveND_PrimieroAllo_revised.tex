...
\end{definition}
%%
Choice sets provide selections of abnormalities that might turn out to be true at a stage {\sf s}. The choice set of an empty set of minimal disjunction of abnormalities is empty. Now we formulate two new marking rules:


\begin{mathpar}
\infer*[right=$\XBox$M]{\Turn{\Gamma;\cdot}{\Delta^{min}}\\
\TurnPrime{\Gamma; \phi^{-}}{\psi} \\
{\forall \Delta^{min}\in \Phi_{s}(\Gamma),\phi\in \Delta^{min}}}
{\TurnMaxPlusOneMA{\Gamma}{\psi}}
\end{mathpar}


\begin{mathpar}
\infer*[right=$\XBox$M2]{\Turn{\Gamma;\cdot}{\Delta^{min}}\\
\TurnPrime{\Gamma; \phi^{-}}{\psi} \\
\TurnPrimePrime{\Gamma; \xi^{-}}{\psi} \\
{\exists \Delta^{min}\in \Phi_{s}(\Gamma),\phi\supseteq \xi\in \Delta^{min}}}
{\TurnMaxTwoPlusOneMA{\Gamma}{\psi}}
\end{mathpar}
