...
\begin{itemize}
  ...
  \item[for \textbf{CluN}$^{M}$] $\Gamma \vdash_{\mathbf{CluN}} \psi\vee \Theta$ and
% $\mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\}) \cap \Theta = \emptyset$
%or every $\mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\})\in \Phi(\Gamma)$,  . Then

\begin{enumerate}
\item there is a $\phi\in \Theta$ such that $\phi\notin \mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\})$, for any
...
\item or for any $\phi\in \Theta$, $\phi\notin \mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\})$ for some $\mathsf{choice_i}(\{\Delta_1, \ldots, \Delta_n\})\in \Phi(\Gamma)$.
...
By Theorem \ref{thm:clun} there is a provable {\sf AdaptiveND} judgement $\Turn{\Gamma;\cdot}{\psi,\Theta}$ and by applying {\sf RC}, we obtain  $\TurnNext{\Gamma;\phi^{-}}{\psi}$ and $\TurnNextNext{\Gamma;\phi'^{-}}{\psi}$, for every $\phi,\phi'\in \Theta$. If either
...
which contradicts the assumption, hence $\TurnChecked{\Gamma;\phi^{-}}{\psi}$ in $P$.
\end{enumerate}
\end{itemize}
