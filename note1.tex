...
The proof-format we propose solves this problem by adding indices to judgements to keep track of stages in the construction of a proof, and by exploiting the fact that since every assumption or premise that is used in a proof-step should explicitly be written down in the place it is used, judgements that are `marked' at a certain stage do not have to be removed because there is no reason to prevent their implicit re-use. Instead, it is the derivation of the same judgement at a later stage that is (or may be) blocked because the original assumption that led to its initial derivation probably no longer holds.

A second feature of the system we propose is that, by relying on a multiple-conclusion judgements, it establishes an explicit connection between the unconditional (and hence paraconsistent) deriviation of certain disjunctions and the conditional deduction of one of their disjuncts through the provisional dismissal of the other disjunct. This formal feature has the additional advantage that it allows us to re-assess certain current debates regarding the question of how one should best approach the question of classical recapture in paraconsistent logics.

We start by characterising our system, and then return to the challenge of classical recapture in Section \ref{sec:recap}
