...
The specific system we describe here is for an inconsistency adaptive logic; a logic that captures the reasoning one obtains when one reasons paraconsistently to avoid triviality in the face of inconsistency, but tries to make up for the deductive weakness of paraconsistent reasoning by provisionally applying classical inference-rules when there is no explicit indication that inconsistencies are involved in that inference. This choice brings us closer to the original motivations for the development of adaptive logic, but also allows us to engage with current philosophical debates and with certain aspects of Graham's work.

The dynamics of retracting earlier lines in a proof can be captured in a rather natural way in linear proof-formats, including standard axiomatic and Fitch-style natural deduction proofs, but is much less straightforward in a tree-like proof-format. Consider, for instance, the following retraction of what turns out to be an instance of the application of \emph{Ex Contradictione Quodlibet}:
...
We start by characterising our system, and only return to the challenge of classical recapture in Section \ref{sec:recap}
...
The semantics of connectives is given in the standard proof-theoretical way by way of Introduction and Elimination Rules in Figure \ref{fig:connectives}. Introduction of $\rightarrow$ corresponds to conditional proof, while its elimination formalises Modus Ponens. Rules for $\wedge$ are standard; notice that $\vee$-Elimination makes the disjunctive reading of the a comma on the right hand-side of the turnstile explicit. $\bot$ can be eliminated by \emph{Ex Falso}, but cannot be introduced. Dually, our paraconsistent negation $\neg$ can be introduced, but not eliminated.
%
%
\begin{figure}[h!]
\begin{mathpar}
...
\begin{figure}[h!]
\begin{mathpar}
\infer*[right=Wleft] {\Turn {\Gamma; \cdot} {\Delta, \phi_{1}} } {\TurnNext {\Gamma, \phi_{2}; \cdot} {\Delta, \phi_{1}}}
\and
\infer*[right=Cleft] {\Turn {\Gamma, \phi_{1}, \phi_{1}; \cdot} {\Delta, \phi_{2}} } {\TurnNext {\Gamma, \phi_{1}; \cdot} {\Delta, \phi_{2}}}
\and
\infer*[right=Eleft] {\Turn {\Gamma, \phi_{1}, \phi_{2}; \cdot} {\Delta, \phi_{3}} } {\TurnNext {\Gamma, \phi_{2}, \phi_{1}; \cdot} {\Delta, \phi_{3}}}
\end{mathpar}
\begin{mathpar}
...
\infer*[right=Cright] {\Turn {\Gamma; \cdot} {\Delta, \phi, \phi}} {\TurnNext {\Gamma; \cdot} {\Delta, \phi} }
\and
            ...
            {\infer*[right=$\vee$I]
                {\infer*[right=$\neg$I]
                    {\infer*[left=Axiom]
                        { }
                        {\Turn {\phi; \cdot} {\phi}}}
                    {\Turn {\emptyset; \cdot} {\phi, \neg \phi}}}
                {\Turn {\emptyset; \cdot} {\phi \vee \neg \phi, \neg \phi}}}
            {\Turn {\emptyset; \cdot} {\phi \vee \neg \phi, \phi \vee \neg \phi}}}
        {\Turn {\emptyset; \cdot} {\phi \vee \neg \phi}}
    \end{mathpar}
    \caption{Excluded Middle}\label{fig:exclmid}
\end{figure}
...
\item the formulation of a rule that allows to derive new formulas that depend on such an adaptive condition.
\end{enumerate}
%
...
\begin{figure}
    \begin{mathpar}
        \infer*[right=RC]{
                        \infer*[right=Cut]{\Turn {\Gamma; \cdot} {\phi_{1}, \phi}\\ {\TurnNext {\phi_{1};\cdot} {\phi_{2}}}}
                        {\TurnNextNext {\Gamma, \phi_{1}; \cdot} {\phi_{2}, \phi}}
                        }
                         {\TurnNextNextNext {\Gamma, \phi_1; \phi^-}{\phi_2}}
        
        \and

        \infer*[right=Cut]
            {\infer*[right=Cut]
            {\Turn {\Gamma; \cdot} {\phi_{1}, \phi}\\ {\TurnNext {\Gamma';\cdot} {\phi_2, \phi'}} \\ {\TurnNextNext {\phi_1, \phi_2;\cdot} {\phi_3}}}
            {\Turn {\Gamma; \cdot} {\phi_{1}, \phi}\\ {\TurnNext {\Gamma', \phi_1 ;\cdot} {\phi_3, \phi'}}}}
            {\infer*[right=RC]
            {\infer*[right=RC]
                {\Turn {\Gamma, \Gamma'; \cdot} {\phi_{3}, \phi, \phi'}}
                {\TurnNext {\Gamma, \Gamma'; \phi^-} {\phi_{3}, \phi'}}}
            {\TurnNext {\Gamma, \Gamma'; (\phi\cup\phi')^-} {\phi_{3}}}}
    \end{mathpar}
    \caption{Derivation of conditional rules}
\end{figure}
